   ...
   // }
    ...
    if (method != NULL) {
        ...
        if (m != -1) {
            ...
            if (cipher == NULL)
                return NULL;
			crypto_t *crypto = (crypto_t *)calloc(1, sizeof(crypto_t));//ss_malloc(sizeof(crypto_t));
           // crypto_t tmp     = {
			crypto->cipher = cipher;
			//(int(*) (buffer_t *, cipher_t *, size_t))crypto->encrypt_all = &stream_encrypt_all;
			crypto->encrypt_all = &stream_encrypt_all;
			crypto->decrypt_all = &stream_decrypt_all;
			crypto->encrypt = &stream_encrypt;
			crypto->decrypt = &stream_decrypt;
			crypto->ctx_init = &stream_ctx_init;
			crypto->ctx_release = &stream_ctx_release;
           // };
...
crypto_parse_key(const char *base64, uint8_t *key, size_t key_len)
{
   ...
   // uint8_t out[out_len];
	std::unique_ptr<uint8_t>out((uint8_t*)malloc(out_len*sizeof(uint8_t)));

    out_len = base64_decode(out.get(), base64, out_len);
    if (out_len > 0 && out_len >= key_len) {
        memcpy(key, out.get(), key_len);
#ifdef SS_DEBUG
   ...
   // char out_key[out_len];
	std::unique_ptr<char>out_key((char*)malloc(out_len*sizeof(char)));
    rand_bytes(key, key_len);
    base64_encode(out_key.get(), out_len, key, key_len);
