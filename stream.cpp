...
int cipher_ctx_set_nonce(cipher_ctx_t *cipher_ctx, uint8_t *nonce, size_t nonce_len, int enc)
{
   unsigned char true_key[MAX_KEY_LENGTH];
    ...
    if (cipher->method == RC4_MD5)
	{
        ...
        crypto_md5(key_nonce, 32, true_key);
        nonce_len = 0;
    } 
	else 
	{
        //true_key = cipher->key;
		memcpy(true_key, cipher->key, cipher->key_len);
    }
    ...
    if (mbedtls_cipher_setkey(evp, true_key, cipher->key_len * 8, (mbedtls_operation_t)enc) != 0) 
	{
...
stream_encrypt_all(buffer_t *plaintext, cipher_t *cipher, size_t capacity)
{
    ...
    if (cipher->method >= SALSA20) {
        crypto_stream_xor_ic((uint8_t *)(ciphertext->data + nonce_len),
                             (const uint8_t *)(plaintext->data + plaintext->idx), (uint64_t)(plaintext->len),
                             (const uint8_t *)nonce,
                             0, cipher->key, cipher->method);
    } else {
        err = cipher_ctx_update(&cipher_ctx, (uint8_t *)(ciphertext->data + nonce_len),
                                &ciphertext->len, (const uint8_t *)(plaintext->data+plaintext->idx),
                                plaintext->len);
    }
...
stream_encrypt(buffer_t *plaintext, cipher_ctx_t *cipher_ctx, size_t capacity)
{
    ...
    if (cipher->method >= SALSA20) {
        ...
        if (padding) {
            ...
            memmove(plaintext->data + padding, plaintext->data+ plaintext->idx, plaintext->len);
			plaintext->idx = 0;
            sodium_memzero(plaintext->data, padding);
        }
        crypto_stream_xor_ic((uint8_t *)(ciphertext->data + nonce_len),
                             (const uint8_t *)(plaintext->data + plaintext->idx),
...
stream_decrypt_all(buffer_t *ciphertext, cipher_t *cipher, size_t capacity)
{
    ...
    uint8_t *nonce = cipher_ctx.nonce;
    memcpy(nonce, ciphertext->data + ciphertext->idx, nonce_len);
	...
	if (cipher_ctx_set_nonce(&cipher_ctx, nonce, nonce_len, 0) < 0) 
	{
		free(tmp.data);
		stream_ctx_release(&cipher_ctx);
		return CRYPTO_ERROR;
    ...
    if (cipher->method >= SALSA20) {
        crypto_stream_xor_ic((uint8_t *)plaintext->data,
                             (const uint8_t *)(ciphertext->data + ciphertext->idx + nonce_len),
...
stream_decrypt(buffer_t* ciphertext, cipher_ctx_t* cipher_ctx, size_t capacity)
{
	...
	if (!cipher_ctx->init) {
		...
		if (left_len > 0) {
			memcpy(cipher_ctx->chunk->data + cipher_ctx->chunk->len, ciphertext->data + ciphertext->idx, left_len);
	...
	if (cipher->method >= SALSA20)
	{
		...
		if (padding) {
			...
			memmove(ciphertext->data + padding, ciphertext->data + ciphertext->idx,
				ciphertext->len);
			ciphertext->idx = 0;
			sodium_memzero(ciphertext->data, padding);
		}
		crypto_stream_xor_ic((uint8_t*)plaintext->data,
			(const uint8_t*)(ciphertext->data+ ciphertext->idx),
...
int stream_encrypt_all_clear_source(buffer_t* plaintext, cipher_t* cipher, size_t capacity)
{
	...
	while (plaintext->len) 
	{
		(plaintext->data + plaintext->idx)[--plaintext->len] = 0;
	}
	free(plaintext->data);
